...
def command_create():
    ...
    (root / 'project.tcl').write_text(textwrap.dedent(f"""
            ...
            set des_files [list \\
                {'\n                '.join(f'"{file}" \\' for file in filesets[FilesetKind.DES])}
            ]
            set sim_files [list \\
                {'\n                '.join(f'"{file}" \\' for file in filesets[FilesetKind.SIM])}
            ]
            set includes [list \\
            ...
            # Add design files
            add_files -fileset sources_1 -norecurse -scan_for_includes $des_files
            add_files -fileset sources_1 $includes
            import_files -fileset sources_1
            set_property include_dirs $includes_imported [get_filesets sources_1]
